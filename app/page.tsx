...
export default function BarberProDashboard() {
  ...
  const renderEventContent = (eventInfo: any) => {
    if (eventInfo.view.type === 'dayGridMonth') {
      return (
        <div className="flex items-center gap-2 px-2 py-1 rounded-md bg-[#1e293b] border border-[#334155] text-slate-300 text-xs w-full overflow-hidden">
    ...
    return (
      <div className="w-full h-full flex items-center bg-[#5b8def] text-slate-900 overflow-hidden px-3 cursor-pointer hover:bg-[#4a7bdd] transition-colors rounded-md">
  ...
  return (
    <div className="flex h-screen bg-[#111111] text-slate-200 overflow-hidden font-sans" dir="rtl">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#111111]">
        ...
        {/* הדר עליון (Header) - מעודכן לפי העיצוב */}
        <header className="h-[72px] flex items-center justify-between px-6 border-b border-white/5 bg-[#111111]">
          
          {/* אמצע - חצים ותאריך */}
          <div className="flex items-center gap-4 flex-1 justify-center">
            <button onClick={() => navigateCalendar('next')} className="p-1.5 hover:bg-white/10 rounded-md text-slate-400 hover:text-white transition-colors">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m9 18 6-6-6-6"/></svg>
            </button>
            <span className="text-xl font-medium w-48 text-center">{currentDateTitle}</span>
            <button onClick={() => navigateCalendar('prev')} className="p-1.5 hover:bg-white/10 rounded-md text-slate-400 hover:text-white transition-colors">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group">
              <button className="flex items-center gap-2 bg-[#1a1a1a] border border-white/10 px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#2a2a2a] transition-colors">
        ...
        {/* יומן */}
        <div className="flex-1 p-4 md:p-6 overflow-hidden calendar-container relative">
          
          {/* כיתוב GMT+02 בתצוגת יום/שבוע */}
          {currentView !== 'dayGridMonth' && (
             <div className="absolute top-6 right-6 z-10 text-[11px] text-slate-400 font-medium">
               GMT+02
             </div>
          )}
      ...
      {/* CSS מותאם אישית שמשכפל את העיצוב בפוטושופ */}
      <style jsx global>{`
        .calendar-container {
        ...
        /* עיצוב התורים - ריווח מהצדדים ועיגול */
        .fc-timegrid-event {
        ...
        .fc-col-header-cell { padding: 16px 0 !important; background: #111111 !important; border-top: none !important; }
        .fc-col-header-cell-cushion { color: #94a3b8 !important; font-weight: 400 !important; font-size: 0.9rem !important; }
        ...
        .fc-timegrid-now-indicator-arrow { border-color: #ef4444 !important; background-color: #ef4444 !important; width: 8px !important; height: 8px !important; border-radius: 50%; margin-top: -4px !important;}
      `}</style>
    </div>
  )
}